...
for dir in /1/apps/*/; do
    [ -d "$dir" ] && if [ -f "$dir"/appmanifest.json ]; then listitemname="$(echo "$dir" | cut -d/ -f4)" && echo "$listitemname"; listitemdir="$(echo "$dir")" && echo "$listitemdir"; fi
#https://askubuntu.com/a/749646
    contents="$(jq ".apps[].\"$listitemname\" = \"$listitemdir\"" list.json)" && \

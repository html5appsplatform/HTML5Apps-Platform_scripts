...
for dir in /1/apps/*/; do
    [ -d "$dir" ] && if [ -f "$dir"/appmanifest.json ]; then listitemname="$(echo "$dir" | cut -d/ -f4)" && echo "$listitemname"; listitemdir="$(echo "$dir")" && echo "$listitemdir"; contents="$(jq ".apps[].\"$listitemname\" = \"$listitemdir\"" list.json)" && \ echo "${contents}" > list.json; fi
